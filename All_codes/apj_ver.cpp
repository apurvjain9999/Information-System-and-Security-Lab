...
void encrypt(const char *key, string P, ofstream &ofile, int m) //Encryption
{
    ...
    for(i=0; i<mod.length(); i++)
    {
        ...
        //cout<<"\t";
        c = n ^ (int)key[j]; //Performing bit by bit xor of characters in key and plain text
        j++;
        if (j == m)
            j = 0;
        //cout<<"\t"<<c<<"\t";
...
void decrypt(const char *key, string C, ofstream &ofile, int m) //Decryption
{
    ...
    for(i=0; i<C.length(); i++)
    {
        ...
        //if (p >= 26)
            //p -= 26;
        //cout<<"\t"<<c<<"\t";
        p = n ^ key[j]; //Performing bit by bit xor of characters in Ciphertext and Key
        j++;
        if (j == m)
            j = 0;
        cout<<(char)(p + 97);
...
int main()
{
    ...
    {
        char exi = 'E';
        while (ans != 'E' || ans != 'e' || ans != 'D' ||  ans != 'D') //User input
        {
        ...
        if (ans == 'E' || ans == 'e')
        {
            ...
            else //Generating key of length 8
            {
                int g;
                for (i=0; i<8; i++)
                {
