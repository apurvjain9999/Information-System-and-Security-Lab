...
ip_file = input("\nEnter input file name: ") #File for Cipher text input
op_file = input("\nEnter output file name: ") #File for saving possible plain text

ip = open(ip_file, "r")

text_freq = {} #For getting frequency of each letter present in the cipher text
st = "A"
while st:
...
s_text_freq = sorted(text_freq.items(), key=lambda x: x[1], reverse=True) #Sorting letters in desceding order of their frequency 
#print(s_text_freq)
...
ip = open(ip_file, "r")
#Generating possible plain text
while st:
    st = ip.readline().upper()
    #print(st)
    wr = ""
